...
이 디렉토리는 KNU Library 앱 개발을 Hyper-Waterfall 방식으로 진행하기 위한 작업 기록이다. 기존 `docs/`는 선별한 Markdown 조사 자료와 과거 보고서 보관소이고, `mydocs/`는 앞으로의 계획, 진행, 검증, 피드백을 남기는 운영 기록이다.
...
- 원본 로그, APK, 캡처, 참조 Python은 Git에 포함하지 않고 필요한 내용만 Markdown으로 정제한다.
