...
Beacon은 iOS와 Android 모두 `modules/beacon-ranging` 로컬 Expo 네이티브 모듈을 통해 동작한다. iOS는 CoreLocation ranging, Android는 BluetoothLeScanner 기반 Kotlin 모듈에서 iBeacon manufacturer data를 파싱한다. Android 실제 기기 검증은 아직 필요하다.
...
| Beacon | 구현됨. iOS CoreLocation native ranging과 Android BluetoothLeScanner native parsing 경로가 있다. | 실제 iOS/Android 기기 검증 |
