...
원본 API 로그, MITM 응답, 참조 Python, APK, 캡처 이미지는 기본 Git 추적 대상에서 제외한다. 2026-04-29 정리 이후 현재 작업 트리에는 해당 원본 파일을 남기지 않고, 필요한 정보만 Markdown 문서로 선별 보존한다.

## 근거

| 항목 | 확인 결과 | 정책 |
|---|---|---|
| `docs/build-1774833724127.apk` | 약 92MB 바이너리 산출물이었다. 현재 삭제됨 | 제외 |
| `docs/mitlogs/` | 원본 API 응답 묶음이었다. 토큰/쿠키/사용자 필드 흔적 가능성이 있어 현재 삭제됨 | 제외 |
| `docs/*.txt` | 원본 HTML/요청/응답 로그 성격이었다. 현재 삭제됨 | 제외 |
| `docs/knulib_api*.py` | 역공학 참조 코드였다. 현재 삭제됨 | 제외 |
| `docs/capture/` | 화면 캡처였다. 개인정보 포함 가능성 미검토 상태라 현재 삭제됨 | 제외 |
...
1. 원본 로그가 다시 필요하면 로컬에서만 재수집하고 필요한 정보만 `mydocs/tech/seat_api_spec.md`로 정제한다.
2. 캡처 이미지는 개인정보 포함 여부를 확인한 뒤 필요한 것만 별도 보관한다.
