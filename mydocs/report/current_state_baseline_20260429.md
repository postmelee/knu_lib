...
| Beacon | 구현됨. iOS/Android 모두 `modules/beacon-ranging` native module 경로를 사용한다. Android 실기기 테스트는 2026-04-30 완료 보고되었다. | `modules/beacon-ranging/`, `src/services/beaconService.ts` |
...
| 완료 | Android 비콘은 native module로 전환되었고 실제 기기 테스트까지 완료 보고되었다. | 후속 제보는 `mydocs/troubleshootings/`에 분리 기록한다. |
...
| M050 | 배포 품질 관문 문서화 완료. Expo SDK patch mismatch와 iOS/Android 실제 실행 검증은 릴리즈 전 차단 항목 |
