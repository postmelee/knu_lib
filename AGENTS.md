...
하이퍼-워터폴 절차의 정형 실행 단위는 `mydocs/skills/`에 둔다. `.agents/skills`는 원본 저장소와 동일하게 `../mydocs/skills`를 가리키는 심볼릭 링크이며, 앱 런타임이나 빌드 입력이 아니다.

모든 하이퍼-워터폴 skill은 `allow_implicit_invocation: false` 기준이다. 작업지시자의 명시 호출 또는 같은 스레드의 단계 승인 없이 자동으로 다음 절차를 진행하지 않는다.

- `mydocs/skills/task-register/SKILL.md`: 신규 GitHub Issue 등록
- `mydocs/skills/task-start/SKILL.md`: 이슈 기반 타스크 시작
- `mydocs/skills/task-stage-report/SKILL.md`: 단계 종료 보고와 단계 커밋
- `mydocs/skills/task-final-report/SKILL.md`: 최종 보고와 draft PR 게시
- `mydocs/skills/pr-merge-cleanup/SKILL.md`: PR merge 후 브랜치·이슈 정리
- `mydocs/skills/external-pr-review/SKILL.md`: 외부 기여자 PR 검토
